...
public class ThumbnailModal implements Modal {

    private final EmbedDatabase embedDatabase;

    public ThumbnailModal(EmbedDatabase embedDatabase) {
    ...
    public void onExecute(ModalInteractionEvent event) {
        ...
        try {
            ...
            if (optionalEmbed.isEmpty()) {
                event.getHook().editOriginal("Etwas ist schiefgelaufen! Bitte Melde dies an <@&1159086761763414046> (ThumbnailModal.java:45)").queue();
                return;
            }
            ...
            embed.setStyle(new Embed.Style(style.color(), style.image(), url, style.timestamp()));

            this.embedDatabase.storeEmbed(embed);

            event.getHook().editOriginalEmbeds(new EmbedBuilder()
                            .setAuthor("Embed Creator")
                            .setDescription("Thumbnail wurde gesetzt!")
